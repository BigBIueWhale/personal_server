...
## 3a. Install OpenAI Codex CLI

Codex is installed with the same opinionated stance as the rest of this repo: one pinned CLI release, one exact model slug, one exact config file.

```bash
bash scripts/00_install_codex_cli.sh        # run as the desktop user, NOT sudo
```

Reference: [`scripts/00_install_codex_cli.sh`](./scripts/00_install_codex_cli.sh). The script does five strict things:
...
There are no profiles, model aliases, fallback models, config merges, stale model-cache fallbacks, old release directories, or tolerated old host Codex processes. Re-running the script accepts only the exact new managed block — alone, or followed solely by Codex's own runtime tables — or an explicit known migration state; any other `~/.codex/config.toml`, stale model cache, release-tree shape, Codex PATH block, or running unmanaged host Codex process causes a hard refusal before managed state changes.
